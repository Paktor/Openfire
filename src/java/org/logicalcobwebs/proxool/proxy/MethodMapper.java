...
public class MethodMapper {
    ...
    private Map<Method, MethodWrapper> cachedConcreteMethods = new HashMap<>();
    ...
    protected Method getConcreteMethod(Method injectableMethod) throws ProxoolException {
        ...
        MethodWrapper concreteMethodWrapper = cachedConcreteMethods.get(injectableMethod);

        // FIX every-time-lookup for missed injectableMethod
        if (concreteMethodWrapper != null && concreteMethodWrapper.getValue() == null) {
        ...
        Method concreteMethod = (concreteMethodWrapper == null) ? null : concreteMethodWrapper.getValue();
        ...
        if (concreteMethod == null) {
            ...
            if (concreteMethod == null) {
                // FIX every-time-lookup for missed injectableMethod
                cachedConcreteMethods.put(injectableMethod, new MethodWrapper(null));

                throw new ProxoolException("Couldn't match injectable method " + injectableMethod + " with any of those " +
                        "found in " + concreteClass.getName());
            }
            // Remember it
            cachedConcreteMethods.put(injectableMethod, new MethodWrapper(concreteMethod));
        }
    ...
    public void overrideConcreteMethod(Method injectableMethod, Method overridenMethod) {
        cachedConcreteMethods.put(injectableMethod, new MethodWrapper(overridenMethod));
    }
